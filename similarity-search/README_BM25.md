...
```bash
python tfidf_bm25_demo.py
```

Expected search result ranking:

```text
c should rank highest
b should rank second
a should rank lowest
```

Because `"bananas"` and `"street"` appear in both `b` and `c`, but document `c` is shorter, so the terms are more concentrated there.
